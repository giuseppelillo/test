...
docker run --name jupyter -p 8888:8888 -t -i --rm -v `pwd`/config.json:/root/.sparkmagic/config.json -v $HOME:/home -v /tmp:/tmp -v /tmp:/.cache -e VAULT_ADDR=$VAULT_ADDR -e VAULT_TOKEN=$VAULT_TOKEN $TAG:$VERSION jupyter lab --notebook-dir=/home --ip='*' --port=8888 --no-browser --allow-root
